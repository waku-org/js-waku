  ...
  implements IConnectionManager
{
   ...
   * 2. If the peer is not part of the configured cluster id, don't dial
  ...
  private async shouldDialPeer(peerId: PeerId): Promise<boolean> {
    ...
    const isSameCluster = await this.isPeerOnSameCluster(peerId);
    if (!isSameCluster) {
      const shardInfo = await this.getPeerShardInfo(peerId);

      log.warn(
        `Discovered peer ${peerId.toString()} with cluster id ${shardInfo?.clusterId}, different from ours (${
          this.clusterId
        }).
            Not dialing.`
      );
  ...
  public async isPeerOnSameCluster(peerId: PeerId): Promise<boolean> {
    ...
    return shardInfo.clusterId == this.clusterId;
  }
