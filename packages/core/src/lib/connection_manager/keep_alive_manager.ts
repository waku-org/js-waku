...
import { isAutoSharding, Logger, RoutingInfo } from "@waku/utils";
...
export class KeepAliveManager implements IKeepAliveManager {
  ...
  private startRelayPing(peerId: PeerId): void {
    ...
    for (const topic of this.relay.pubsubTopics) {
      ...
      const routingInfo = isAutoSharding(this.networkConfig)
        ? RoutingInfo.fromContentTopic(
            RelayPingContentTopic,
            this.networkConfig
          )
        : RoutingInfo.fromPubsubTopic(topic, this.networkConfig);
