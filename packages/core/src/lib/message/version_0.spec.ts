...
import { contentTopicToPubsubTopic, RoutingInfo } from "@waku/utils";
...
const testPubsubTopic = contentTopicToPubsubTopic(testContentTopic);
const testRoutingInfo = RoutingInfo.fromContentTopic(testContentTopic, {
  clusterId: 0,
  numShardsInCluster: 12
});
...
describe("Ensures content topic is defined", () => {
  ...
  it("Encoder throws on empty string content topic", () => {
    const wrapper = function (): void {
      createEncoder({ contentTopic: "", routingInfo: testRoutingInfo });
    };
...
describe("Sets sharding configuration correctly", () => {
  it("uses static shard pubsub topic instead of autosharding when set", async () => {
    ...
    const staticshardingEncoder = createEncoder({
      contentTopic: ContentTopic,
      routingInfo: RoutingInfo.fromShard(0, { clusterId: 0 })
    });
