...
import { fetchAndValidateNodes } from "./fetch_nodes.js";
...
describe("Fetch nodes", function () {
  it("Get Nodes", async function () {
    ...
    const res = await fetchAndValidateNodes(5, getNode);

    expect(res.length).to.eq(2);
    expect(res[0].peerId!.toString()).to.not.eq(res[1].peerId!.toString());
  });

  it("Stops search when maxSearch is reached", async function () {
    ...
    const res = await fetchAndValidateNodes(2, getNode);
  ...
  it("Stops search when 2 null results are returned", async function () {
    ...
    const res = await fetchAndValidateNodes(10, getNode);
