...
import { isWireSizeUnderCap, Logger } from "@waku/utils";
...
describe("Waku Relay [node only]", () => {
  ...
  describe("Custom pubsub topic", () => {
    ...
    it("Publishes wire size <= 1 MB and rejects others", async function () {
      this.timeout(10000);
      const MB = 1024 ** 2;
      ...
      const payloadSizes = [1 * MB - 65536, 1 * MB, 1 * MB + 65536, 2 * MB];
      for (const payloadSize of payloadSizes) {
        const message = {
          payload: generateRandomUint8Array(payloadSize)
        };

        const encodedMesssage = await CustomEncoder.toWire(message);
        const sendResult = await waku1.relay.send(CustomEncoder, message);

        if (isWireSizeUnderCap(encodedMesssage)) {
